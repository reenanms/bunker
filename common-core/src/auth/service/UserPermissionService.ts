import { UserPermission } from "../../auth/entity/UserPermission";
...
export class UserPermissionService {
  ...
  private async getRoutePermissions(route: string): Promise<UserPermission[]> {
    ...
    const possibleRoutePermission =
      await this.buildPossibleRoutePermission(routeLowerCase);

    const permissions = this.userPermissions.filter((permission) => {
      const currentRouteLowerCase = permission.route.toLowerCase();
      for (const route of possibleRoutePermission) {
        if (route == currentRouteLowerCase) return true;
      }
      return false;
    });
  ...
  private async hasPermission(
    routePermissions: UserPermission[],
    permission: string,
  ): Promise<boolean> {
    const PERMISSION_ALL = "*";

    for (const routePermission of routePermissions) {
      if (routePermission.permission == PERMISSION_ALL) return true;
      if (routePermission.permission == permission) return true;
    }
  ...
  public async hasAccess(route: string, permission: string): Promise<boolean> {
    ...
    const userHasPermission = await this.hasPermission(
